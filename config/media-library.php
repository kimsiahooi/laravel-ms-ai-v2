...
use App\Support\Media\UlidFileNamer;
...
return [
     ...
     */
    /*
     * `assets`, not `public`: nothing here may be served straight off the filesystem.
     * A product photo belongs to one workspace, and the `public` disk is a symlink into
     * the docroot — anyone with the URL would read it, signed in or not. See
     * config/filesystems.php for why this disk is central and un-suffixed.
     */
    'disk_name' => env('MEDIA_DISK', 'assets'),
     ...
     */
    /*
     * False, deliberately. QUEUE_CONNECTION is `database` and nothing in this app runs
     * a worker, so a queued conversion is a thumbnail that never appears — the row
     * renders a broken image and no error is raised anywhere. Generating inline costs
     * the upload request a few hundred milliseconds, once, and is what makes the
     * thumbnail exist by the time the page that shows it is rendered.
     */
     ...
     */
    // Null, not the package's default: the class it names ships with Media Library Pro,
    // which this app does not have. Referencing a class that is not installed is a
    // PHPStan failure in config/, and the only code paths that read this key call
    // MediaLibraryPro::ensureInstalled() first, so they were never reachable anyway.
    ...
    /*
     * Files are stored under a fresh ULID rather than the name they were uploaded with.
     * Media for one kind of record shares a folder (see TenantPathGenerator), so the
     * uniqueness has to be in the filename — and the uploaded name never reaches the
     * filesystem, which is the other half of the reason. The original is kept in
     * `media.name`.
     */
    ...
    /*
     * Scratch space for image conversions. Set explicitly, and that is the whole point.
     *
     * Left null, the package falls back to `storage_path('media-library/temp')` — and it
     * resolves that INSIDE the request, by which time stancl's FilesystemTenancyBootstrapper
     * has repointed storage_path() at `storage/tenant{id}`. So every workspace that ever
     * uploaded a photo left a `storage/tenantdemo/media-library/temp` behind: an empty
     * directory per tenant, at the storage root, that nothing ever reads.
     *
     * A value written here is resolved once at BOOT, before any tenant is identified, so it
     * stays central no matter which workspace is being served. Every other storage_path() in
     * this app is already in a config file for the same reason; this was the only one being
     * resolved late.
     *
     * One shared directory is safe to share between workspaces: medialibrary appends a
     * Str::random(32) segment per operation and deletes it afterwards (see
     * Spatie\MediaLibrary\Support\TemporaryDirectory), so two concurrent conversions never
     * meet and nothing accumulates.
     */
